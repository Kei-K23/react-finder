...
export const useFilesystemStore = create<UseFilesystemStoreType>()(
  persist(
    (set, get) => ({
      ...
      addNewNode: (parentNodeName: string, newNode: Node) =>
        set((state) => ({
          nodes: addNode(state.nodes, parentNodeName, newNode),
        })),
      addNewNodeForLeft: (newNode: Node) =>
        set((state) => ({
          nodes: [...state.nodes, newNode],
        })),
      updateNode: (nodeName: string, updatedNode: Partial<Node>) =>
        set((state) => ({
          nodes: updateNode(state.nodes, nodeName, updatedNode.name!),
        })),
